import { DOMParser, XMLSerializer, onErrorStopParsing, type Element, type Node } from '@xmldom/xmldom';
...
function sanitizeElement(el: Element): void {
  ...
  for (let i = 0; i < el.attributes.length; i++) {
    ...
    if (attr) attrNames.push(attr.name);
  }
  ...
  for (const name of attrNames) {
    ...
    if (URL_ATTRS.has(localName)) {
      ...
      if (!isSafeReference(value)) el.removeAttribute(name);
      continue;
    }
    ...
    const value = el.getAttribute(name) ?? '';
    if (EXTERNAL_URL.test(value)) el.removeAttribute(name);
  }
}
