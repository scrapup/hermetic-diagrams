 ...
 */
const DEFERRED_FORMATS: ReadonlySet<string> = new Set([
  'mermaid',
  'bpmn',
  'excalidraw',
  'bytefield',
 ...
 */
export function validateFormat(input: { format: unknown; output?: unknown }): ValidatedFormat {
  const { format, output } = input;

  if (typeof format !== 'string' || format.trim() === '') {
    throw new HermeticError(
      'INVALID_FORMAT',
      `Missing "format". Supported formats: ${INPUT_FORMATS.join(', ')}.`,
    );
  }
  ...
  if (!isInputFormat(normalizedFormat)) {
    if (DEFERRED_FORMATS.has(normalizedFormat)) {
      throw new HermeticError(
        'INVALID_FORMAT',
        `Format "${normalizedFormat}" is not available in this MVP cycle (it requires browser components). Supported formats: ${INPUT_FORMATS.join(', ')}.`,
      );
    }
    throw new HermeticError(
      'INVALID_FORMAT',
      `Unknown format "${normalizedFormat}". Supported formats: ${INPUT_FORMATS.join(', ')}.`,
    );
  }

  let normalizedOutput: OutputFormat = 'svg';
  if (output !== undefined) {
    if (typeof output !== 'string' || !isOutputFormat(output.trim().toLowerCase())) {
      const shown = typeof output === 'string' ? output : typeof output;
      throw new HermeticError(
        'INVALID_FORMAT',
        `Unknown output "${shown}". Supported outputs: ${OUTPUT_FORMATS.join(', ')}.`,
      );
    }
    normalizedOutput = output.trim().toLowerCase() as OutputFormat;
  }
