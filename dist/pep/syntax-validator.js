import { JSON_FORMATS } from '../domain.js';
...
function rejectSyntax(message, detail) {
    throw new HermeticError('INVALID_SYNTAX', message, detail);
}
function validateJsonNotation(format, source) {
    try {
        JSON.parse(source);
    }
    catch (err) {
...
function validateGraphviz(source) {
    ...
    if (open !== close)
        rejectSyntax('Unbalanced braces in the GraphViz source.');
}
/**
 ...
 */
export function validateSyntax(format, source) {
