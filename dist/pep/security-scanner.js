...
function reject(detail) {
    throw new HermeticError('EXTERNAL_REFERENCE', 'The source references an external resource, which is not allowed. No render was attempted.', detail);
}
function scanCommonXml(source) {
    if (DTD_TOKEN.test(source)) {
        reject('XML DTD / external entity declaration detected (XXE vector).');
    }
}
function scanPlantUml(source) {
    // `%getenv` would exfiltrate environment values into the render.
    if (/%getenv\b/i.test(source))
        reject('PlantUML `%getenv` is not allowed.');
    for (const rawLine of source.split('\n')) {
        const line = rawLine.trim();
        // `!includeurl` is remote by definition.
        if (/^!includeurl\b/i.test(line))
            reject('Remote `!includeurl` directive.');
        ...
        if (/^!theme\b/i.test(line) && URL_SCHEME.test(line))
            reject('Remote `!theme ... from <url>`.');
        // Sprites referenced by URL: `<img:https://...>`.
        if (/<img:\s*[^>]*?(?:https?|ftps?|file):\/\//i.test(line))
            reject('Remote sprite `<img:url>`.');
    }
}
function scanD2(source) {
    for (const rawLine of source.split('\n')) {
        const line = rawLine.trim();
        // Remote spread-import: `...@https://...` or `@import ... url`.
        if (/@\S*/.test(line) && URL_SCHEME.test(line))
            reject('Remote D2 `@import`.');
        // `icon: https://...`.
        if (/\bicon\s*:/i.test(line) && URL_SCHEME.test(line))
            reject('Remote D2 `icon` URL.');
    }
}
function scanGraphviz(source) {
    for (const rawLine of source.split('\n')) {
        const line = rawLine.trim();
 ...
 */
export function assertSourceSize(source, maxSourceBytes) {
    const byteLength = Buffer.byteLength(source, 'utf8');
    if (byteLength > maxSourceBytes) {
        throw new HermeticError('TOO_LARGE', `Source exceeds the maximum size of ${maxSourceBytes} bytes (got ${byteLength}).`);
    }
}
/**
 ...
 */
export function scanSecurity(format, source, maxSourceBytes) {
